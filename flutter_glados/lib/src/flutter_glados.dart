...
class Glados<T> {
  ...
  void testWidgets(
    ...
    /// by gradually increasing the size. Returns the first value where the
    /// property is broken or null if no value was found.
    FutureOr<Shrinkable<T>?> explorePhase(
        flutter_test.WidgetTester tester) async {
      var count = 0;
      var size = explore.initialSize;
      bodyWithTester(input) => body(tester, input);

      while (count < explore.numRuns) {
        stats.exploreCounter++;
        final input = generator(explore.random, size.ceil());
        if (!await succeeds(bodyWithTester, input.value)) {
          return input;
        }
    ...
    /// Shrinks the given value repeatedly. Returns the shrunk input.
    FutureOr<T> shrinkPhase(Shrinkable<T> initialErrorInducingInput,
        flutter_test.WidgetTester tester) async {
      bodyWithTester(input) => body(tester, input);
      var input = initialErrorInducingInput;
      ...
      while (true) {
        for (final shrunkInput in input.shrink()) {
          stats.shrinkCounter++;
          if (!await succeeds(bodyWithTester, shrunkInput.value)) {
            input = shrunkInput;
            continue outer;
          }
        }
    ...
    flutter_test.testWidgets(
      ...
      '${explore.numRuns == 1 ? 'input' : 'inputs'})',
      (tester) async {
        final errorInducingInput = await explorePhase(tester);
        if (errorInducingInput == null) return;

        final shrunkInput = await shrinkPhase(errorInducingInput, tester);
        print('Tested ${stats.exploreCounter} '
            '${stats.exploreCounter == 1 ? 'input' : 'inputs'}, shrunk '
            '${stats.shrinkCounter} ${stats.shrinkCounter == 1 ? 'time' : 'times'}.'
            '\nFailing for input: $shrunkInput');
        final output =
            body(tester, shrunkInput); // This should fail the test again.
...
class Glados2<First, Second> {
  ...
  void testWidgets(
    String description,
    GladosWidgetTester2<First, Second> body, {
    String? testOn,
    test_package.Timeout? timeout,
    dynamic skip,
    dynamic tags,
...
class Glados3<First, Second, Third> {
  ...
  void testWidgets(
    String description,
    GladosWidgetTester3<First, Second, Third> body, {
    String? testOn,
    test_package.Timeout? timeout,
    dynamic skip,
    dynamic tags,
